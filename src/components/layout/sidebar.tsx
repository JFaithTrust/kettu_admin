...
const Sidebar = () => {
  ...
  return (
    <motion.div
      ...
      className={clsx("flex flex-col justify-between border-r-[1px] border-solid border-gray-200 dark:border-secondary pb-4", {
        "px-3": open,
      })}>
      <div className={"flex flex-col space-y-4"}>
        <div className={"p-4 border-b-[1px] border-solid border-gray-200 dark:border-secondary"}>
        ...
        <div className={"flex flex-col space-y-1"}>
          {sidebarLinks.map((link) => (
            <Link href={link.href} key={link.label}
                  className={clsx("flex justify-between py-2 px-3 rounded-md hover:bg-slate-200 dark:hover:bg-secondary cursor-pointer", {
                    "bg-slate-200 border-solid border-r-[6px] border-primary dark:bg-secondary": pathname === link.href,
                    "mx-3": !open,
                  })}>
